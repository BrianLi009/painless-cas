...
void
Solver::detachClause(CRef cr, bool strict)
{
	const Clause& c = ca[cr];
	assert(c.size() > 1);
	OccLists<Lit, vec<Watcher>, WatcherDeleted>& ws = c.size() == 2 ? watches_bin : watches;
...
bool
Solver::satisfied(const Clause& c) const
{
	for (int i = 0; i < c.size(); i++)
		if (value(c[i]) == l_True)
			return true;
...
void
Solver::analyze(CRef confl, vec<Lit>& out_learnt, int& out_btlevel, int& out_lbd)
{
	int pathC = 0;
	Lit p = lit_Undef;
	...
	do {
		...
		// For binary clauses, we don't rearrange literals in propagate(), so check and make sure the first is an
		// implied lit.
	...
	if (ccmin_mode == 2) {
		...
		for (i = 1; i < out_learnt.size(); i++)
			abstract_level |=
				abstractLevel(var(out_learnt[i])); // (maintain an abstraction of levels involved in conflict)
...
bool
Solver::binResMinimize(vec<Lit>& out_learnt)
{
...
|________________________________________________________________________________________________@*/
void
Solver::analyzeFinal(Lit p, vec<Lit>& out_conflict)
{
	out_conflict.clear();
	out_conflict.push(p);
...
void
Solver::uncheckedEnqueue(Lit p, CRef from)
{
...
|________________________________________________________________________________________________@*/
bool
Solver::simplify(bool do_stamping)
{
	assert(decisionLevel() == 0);
...
void
Solver::removeClauseHack(CRef cr, Lit watched0, Lit watched1)
{
	assert(ca[cr].size() >= 2);
...
// TODO: needs clean up.
void
Solver::safeRemoveSatisfiedCompact(vec<CRef>& cs, unsigned valid_mark)
{
	int i, j, k, l;
	for (i = j = 0; i < cs.size(); i++) {
		Clause& c = ca[cs[i]];
...
void
Solver::getAssumptions(vec<Lit>& lits)
{
...
void
Solver::setStrengthening(bool b)
{
	strengthening = b;
}

double
Solver::progressEstimate() const
{
	double progress = 0;
	double F = 1.0 / nVars();
...
bool
Solver::stampAll(bool use_bin_learnts)
{
	// Initialization.
	int stamp_time = 0;
	int nLits = 2 * nVars();
...
bool
Solver::implExistsByBin(Lit p, bool use_bin_learnts) const
{
	assert(value(p) == l_Undef);
